...
public class BondedDihedral : MonoBehaviour
{
    ...
    public static List<Vector3> F_d(float dangle_cos, List<GameObject> dangle_beads, float K_d, float phi_d, List<Vector3> distances) {
        ...
        Vector3 F1 = -K_d * (MathF.Sin(d_angle) - phi_d) * 
            ((Mathf.Pow(Vector3.Cross(r12, r23). 2) * Vector3.Dot(r23, r23) * r34) )/
            (Mathf.Pow((Vector3.Cross(r12, r23)).magnitude, 3) * Vector3.Cross(r23, r34));
        Vector3 F2 = -K_d * (MathF.Sin(d_angle) - phi_d) * 
            (()/
            (Mathf.Pow(Vector3.Cross(r12, r23).magnitude, 3) * Mathf.Pow(Vector3.Cross(r23, r34).magnitude, 3)));
        Vector3 F3 = -K_d * (MathF.Sin(d_angle) - phi_d) * 
            (()/
            (Mathf.Pow(Vector3.Cross(r12, r23).magnitude, 3) * Mathf.Pow(Vector3.Cross(r23, r34).magnitude, 3)));
        Vector3 F4 = -K_d * (MathF.Sin(d_angle) - phi_d) * 
            (()/
            (Vector3.Cross(r12, r23) * Mathf.Pow((Vector3.Cross(r23, r34)).magnitude, 3)));
        // Vector3 F1 = 10 * r1;
        // Vector3 F2 = 10 * r2;
        // Vector3 F3 = 10 * r3;
        // Vector3 F4 = 10 * r4;
