...
public class NonBondedForce : MonoBehaviour
{
    ...
    public static List<Vector3> Fnb(float sigma, float epsilon, List<Vector3> distances) {
        ...
        Vector3 F1 = (48 * epsilon * Mathf.Pow(sigma, 12) / Mathf.Pow(r.magnitude, 13) - 24 * epsilon * Mathf.Pow(sigma, 6) / Mathf.Pow(r.magnitude, 7)) * dir;
        Vector3 F2 = (-48 * epsilon * Mathf.Pow(sigma, 12) / Mathf.Pow(r.magnitude, 13) - 24 * epsilon * Mathf.Pow(sigma, 6) / Mathf.Pow(r.magnitude, 7)) * dir;
    ...
    public static Vector3 F_nb(GameObject tgtBead, List<GameObject> exclude, List<GameObject> beads)
    {
        ...
        foreach (var (bead, i) in beads.Except(exclude).Select((x, i) => (x, i)))
        {
            ...
            var F_LJ = 48 * intLevel * Mathf.Pow(effSize, 12) / Mathf.Pow(distance, 13) - 24 * intLevel * Mathf.Pow(effSize, 6) / Mathf.Pow(distance, 7);
            F_nb += F_LJ * dir;
        }
